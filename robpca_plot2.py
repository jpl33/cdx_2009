...
intervals=4#math.ceil((last_ts-first_ts)/time_interval)
...
#    mcd_lst=list(df2.loc[df_clean.loc[df_clean.mcd==True].index.values,'_id'])
    ...
    doc =collection_bins.find({'pcap_dir':pcap_dir,'index':index})
    bin_df=pd.DataFrame(list(doc))
    ...
    for name, group in conn_groups:
        if name==False:
            ...
            ax11.axhline(y= bin_df['OD_threshold'][0] )
            ax11.axvline(x= bin_df['SD_threshold'][0]  )                       
    ...
    fig.savefig(str(pcap_dir+'_'+str(index)+'_biplot.png'),bbox_inches='tight')  
    ...
    for ll,k in zip(lambda_c,'b'):#grc'):#mykw'):
        ...
        df_target['attack']=df.attack_bool.apply(lambda x: 1 if x==True else 0).astype(int)
    ...
    plt.show()
    fig_plot.savefig(pcap_dir+'_index_'+str(index)+'_roc.png')
