...
db = client['local']
collection_pcap = get_db()[pcap_dir+'_train_bro']
...
def unset_vars():
    srv='conn'
    service_collection_name=pcap_dir+'_'+srv+'_pp'
...
#            gtemp[feature+'_pair_freq']=gtemp[feature+'_pair_freq']
    ...
    try:
        ...
        error=str(e)+':pcap_dir='+pcap_dir+':bin='+str(bin)+':error on PcaGrid'
    ...
    lambda_mat=e_vals[:k]*np.identity(k)    
    ...
    #df["mcd"]=False
    #df.loc[df3_n.iloc[H1==1].index.values,'mcd']=True
