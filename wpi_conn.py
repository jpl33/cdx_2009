...
for fl in dl: 
    with open(home_dir+wpi_dir +fl,'r') as wpi_f:
        ...
        if len(udp_ll)>0:
            ...
            is_portmap_vec=pd.DataFrame(is_portmap_vec.fillna(0).apply(lambda x:1 if x==111 else 0).values,columns=['is_portmap'] )
        else:
        ...
        if len(rpc_ll)>0:
            is_sadmind_vec=pd.DataFrame(file_df['layers.rpc.rpc_rpc_program'].astype(float).apply(lambda x: 1 if x ==100232 else 0).values,columns=['is_sadmind'])
        else:
        ...
        if len(tcp_ll)>0:
            ...
            is_telnet_vec=pd.DataFrame(src_port_tcp_vec.mask(src_port_tcp_vec!=23,dst_port_tcp_vec).fillna(0).astype(int).apply(lambda x: 1 if x ==23 else 0).values,columns=['is_telnet'])
            is_ftp_vec=pd.DataFrame(src_port_tcp_vec.mask(src_port_tcp_vec!=21,dst_port_tcp_vec).fillna(0).astype(int).apply(lambda x: 1 if x ==21 else 0).values,columns=['is_ftp'])
            is_http_vec=pd.DataFrame(src_port_tcp_vec.mask(src_port_tcp_vec!=80,dst_port_tcp_vec).fillna(0).astype(int).apply(lambda x: 1 if x ==80 else 0).values,columns=['is_http'])
        else:
