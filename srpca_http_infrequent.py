...
service_categorical_features=['method' 
...
for pdir in pcap_dirs:  
    ...
    for index in range(intervals):
        ...
        category_features= [ ft  for ft in service_categorical_features if ft in df.columns]
    ...
    #    # # grouping http flows by orig-resp pairs         
        ...
        for ll in lz:
            dfft=pd.DataFrame()
            gbl.append(ll[1])
            gbcc=df2.groupby(gbl)
            gbcc_vals=[len(vv.values) for vv in gbcc.groups.values()]
            gbt=tuple(gbl)
            dfft['key']=gbcc.groups.keys()
            dfft['values']=gbcc_vals
            dfft['level']=len(gbl)
            for an1 in anomaly_keys:
                for an2 in dfft.key:
                    if subset_tupl(an1,an2)==0:
                        dfft.loc[dfft.key==an2,'values']=-1
                                            
            dff2=dff2.append(dfft)
            if len(gbl)>1:
                for kko in old_keys:
                    dff2.loc[(dff2.level==len(gbl)-1)&(dff2.key==kko),'values']=0
            if dfft.loc[dfft['values']<support_th*df_cnt,'key'].shape[0]>1:    
        ...
        for kk in lz:
            ...
            for kk2 in gbcc2.groups.keys():
                g1=gbcc2.get_group(kk2)
                dff2.loc[dff2.key==kk2,'conns']=g1.shape[0]
                dff2.loc[dff2.key==kk2,'attacks']=g1.loc[g1.attack_bool==True].shape[0]
                if dff2.loc[dff2.key==kk2,'values'].values[0]==0 :
                    dff2.loc[dff2.key==kk2,'outlyr_index']=-1
                else:
                    dff2.loc[dff2.key==kk2,'outlyr_index']=1/(dff2.loc[dff2.key==kk2,'values']*dff2.loc[dff2.key==kk2,'level'])        
